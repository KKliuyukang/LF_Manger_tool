...
window.DataAggregator = {
    ...
    removeDuplicates: function() {
        console.log('🔄 开始智能检测和移除重复数据...');
        
        try {
            if (!gameData.financeData?.accountData) {
                console.log('❌ 没有账户数据可处理');
            ...
            // 处理每个账户
            Object.entries(accountData).forEach(([accountId, monthsData]) => {
                const account = gameData.financeData.accounts[accountId];
                const accountName = account?.name || accountId;
                
                console.log(`📋 检查账户: ${accountName}`);
                ...
                Object.entries(monthsData).forEach(([monthKey, monthData]) => {
                    ...
                    const originalCount = monthData.expenses.length;
                    const uniqueExpenses = [];
                    const seenKeys = new Set();
                    let removedInThisMonth = 0;
                    
                    monthData.expenses.forEach(expense => {
                        // 创建更严格的重复检测键
                        const key = this.createExpenseKey(expense);
                        
                        if (seenKeys.has(key)) {
                            removedInThisMonth++;
                            console.log(`  ❌ 移除重复项: ${expense.name} - ${expense.amount} ${expense.currency}`);
                        } else {
                            seenKeys.add(key);
                    ...
                    if (removedInThisMonth > 0) {
                        monthData.expenses = uniqueExpenses;
                        totalRemovedCount += removedInThisMonth;
                        
                        cleanupDetails.push({
                            accountId,
                            accountName,
                            month: monthKey,
                            removedCount: removedInThisMonth,
                            originalCount,
                            finalCount: uniqueExpenses.length
                        });
                        
                        console.log(`  ✅ ${monthKey}: 移除 ${removedInThisMonth} 个重复项 (${originalCount} -> ${uniqueExpenses.length})`);
                    }
                });
            });
            
            if (totalRemovedCount > 0) {
                console.log(`✅ 总共移除 ${totalRemovedCount} 个重复数据项`);
                
                // 重新汇总数据以确保一致性
                this.aggregateAllAccounts();
                ...
                // 更新相关界面
                setTimeout(() => {
                    if (window.AccountManager?.refreshAllFinanceDisplays) {
                        window.AccountManager.refreshAllFinanceDisplays();
                    }
                }, 100);
            ...
            console.error('❌ 移除重复数据失败:', error);
            throw error;
        }
    },
    
    // 新增：创建支出项目的唯一键
    createExpenseKey: function(expense) {
        // 使用名称、金额和货币创建键，忽略大小写和多余空格
        const name = (expense.name || '').trim().toLowerCase();
        const amount = expense.amount || 0;
        const currency = (expense.currency || 'AUD').toUpperCase();
        
        // 四舍五入金额到小数点后2位以避免浮点数精度问题
        const roundedAmount = Math.round(amount * 100) / 100;
        
        return `${name}_${roundedAmount}_${currency}`;
    },
    
    // 新增：自动检测潜在重复数据
    autoDetectDuplicates: function() {
        console.log('🔍 自动检测潜在重复数据...');
        ...
        Object.entries(monthlyExpenses).forEach(([monthKey, expenses]) => {
            ...
            expenses.forEach((expense, index) => {
                ...
                expenseGroups.get(key).push({ ...expense, index });
            });
        ...
        console.log(`🔍 检测到 ${duplicateGroups.length} 组潜在重复数据`);
